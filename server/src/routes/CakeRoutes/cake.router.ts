...
import Cake from "../../models/CakeModel/Cake";
...
cakeRouter.get("/", async (_req: Request, res: Response) => {
    try {
        const cakes = (await collections
            .cakes!.find({})
            .toArray()) as unknown as typeof Cake[];
...
// GET by id
cakeRouter.get("/:_id", async (req: Request, res: Response) => {
...
// PUT
cakeRouter.put("/:_id", async (req: Request, res: Response) => {
    const id = req?.params?.id;
...
// DELETE
cakeRouter.delete("/:_id", async (req: Request, res: Response) => {
    const id = req?.params?.id;
